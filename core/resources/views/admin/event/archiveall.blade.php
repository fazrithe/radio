        ...
        <div class="col-lg-12">
            <div class="card b-radius--10 ">
                                ...
                                @forelse($archives as $archive)
                                    <tr>
                                        ...
                                        <td data-label="@lang('User')">
                                            <div class="user">
                                                <div class="thumb"><img
                                                        src="{{ getImage('assets/images/rj_image/' . $archive->jockey_image) }}"
                                                        alt="@lang('image')"></div>
                                            </div>
                                        </td>

                                        <td data-label="@lang('Event_name')">{{ __($archive->event_name) }}</td>
                                        <td data-label="@lang('Event Day')">{{ __($archive->week_name) }}</td>

                                        <td data-label="@lang('Start_time')">{{ \Carbon\Carbon::parse($archive->start_time)->format('H:i a') }}</td>
                                        <td data-label="@lang('End_time')">{{ \Carbon\Carbon::parse($archive->end_time)->format('H:i a') }}</td>
                                        <td data-label="@lang('Status')">{{
                                            ($archive->url == null && $archive->input_file == null) ? 'Add url / File' : 'Added'}}
                                        </td>
                                        <td data-label="@lang('Action')">

                                            <a href="{{ route('admin.event.archive.details', $archive->id) }}" class="icon-btn mr-1"
                                            ...
                                            title="Delete" data-original-title="Delete" data-event="{{$archive->id}}" data-url="{{route('admin.archive.delete',$archive->id)}}">
                ...
                <div class="card-footer py-4">
                    {{ $archives->links('admin.partials.paginate') }}
                </div>
            </div><!-- card end -->
        </div>
...
@push('breadcrumb-plugins')
    <form action="{{ route('admin.archive.event.search',request('search'))}}" method="GET" class="form-inline float-sm-right bg--white">
