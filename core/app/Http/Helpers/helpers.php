...
function imagePath()
{
    ...
    $data['rj_image'] = [
        'path'  => 'assets/images/rj_image',
        'size'  => '350x400'
    ];

    $data['rj_gallary'] = [
        'path'  => 'assets/images/rj_gallary',
        'size'  => '400x400'
    ];
...
function frontendImage($folder = '')
{
    return 'assets/images/frontend/' . $folder . '/';
}
function sectionImage($folder = '', $image = '',$size='400x400')
{
    if (file_exists(frontendImage($folder) . $image) && $image != null) {
        return asset(frontendImage($folder) . $image);
    }
    return route('placeholderImage', $size);;
}
